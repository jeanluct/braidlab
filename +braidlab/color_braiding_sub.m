...
%COLOR_BRAIDING_SUB   Braid generators for subbraid.
...
% contents corresponding to the i_th and j_th designated punctures which are
% in the complete crossing structure, CROSS_CELL.
...
% and creation of the generator sequnce, GEN, and the time of crossing, TCR,
% vectors.
...
for I = 1:n
  for J = 1:n
    if size(cross_cell_sub{I,J},1)~=0
      t_cross = [t_cross; cross_cell_sub{I,J} ...
		 ones(size(cross_cell_sub{I,J},1),1)*I ...
		 ones(size(cross_cell_sub{I,J},1),1)*J];
    end
  end
end
...
for i = 1:size(t_cross,1)
  ...
  if Iperm(ind_1+1) == t_cross(i,4)
    ...
    error('BRAIDLAB:color_braiding_sub:badcrossing', ...
	  't_cross inconsistency.')
  end
end
