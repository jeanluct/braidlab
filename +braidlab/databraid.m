...
classdef databraid < braidlab.braid
  ...
  methods
    ...
    %
    %   * Method - [ {'proj'} | 'nonproj' ] - Choice of algorithm for
    %   computing the length of the loop after action of the
    %   braid. 
    %   
    %     'proj' uses projectivized coordinates which makes it
    %     more suitable for very large braids, at the possible (small) expense
    %     in numerical accuracy. (See braid.entropy for internals).
    %
    %     'nonproj' uses non-projectivized coordinates which may make
    %     it slow or unusable for very long braids, but its numerical
    %     precision should be dictated only by precision of evaluation
    %     of natural logarithm. (See braid.complexity for
    %     internals) 
    %
