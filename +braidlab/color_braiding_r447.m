...
% analyzed one the code loops through each of the strands and compares it to
% every strand with a higher index
...
for I = 1:n
  ...
  for J = I+1:n
    ...
    % will correspond to a change in sign of PERM.

    perm = sign(x_traj1-x_traj2);
    permZeroIndex = find(perm==0);
    perm(permZeroIndex) = perm(permZeroIndex-1);

    % For each time step up to the last one the code will determine if there
    % was a crossing.  Two things have to be taken into account.  If the two
    % points happen to have the same x-coordinate than either a crossing is
    % occuring at that moment or the points are colliding (i.e. have the
    % same y-coordinate).  If the trajectories have the same x-coordinate
    % the value of PERM at this instant will be 0.  Otherwise crosses only
    % occur when there has been a change in the value of PERM from one time
    % step to the next.

    for i = 1:length(perm)-1
      if perm(i+1) ~= perm(i) % true if the order of the trajectories changed
	% Function INTERPCROSS calculates the interpolated time of crossing
...
for I = 1:n % Cycle through all cells
  for J = 1:n
    if size(cross_cell{I,J},1)~=0 % If the cell is not empty add information
      t_cross = [t_cross; cross_cell{I,J} ...
...
% caluclated earlier is then applied to get the generator value.
...
for i = 1:size(t_cross,1)
  ...
  if Iperm(ind_1+1) == t_cross(i,4)    % If the higher strand is in fact the
				       % next strand to the right apply the
                                       % crossing.
    Iperm(ind_1:ind_1+1) = [t_cross(i,4) t_cross(i,3)]; % Update the index
                                                        % vector
      ...
      tcr(i) = t_cross(i,1);       % save the time of crossing
       ...
       fs = ['t_cross inconsistency at crossing %d, time %f' ...
	     ' with permutation [' num2str(Ip) '].'];
       msg = sprintf(fs,i,t_cross(i,2));
