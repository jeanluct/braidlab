...
of its closure.  For instance, the trefoil knot is given by the closure of the
\index{knot!trefoil}%
...
ans(z) = - z^(+1) + 3 - z^(-1)
\end{lstlisting}
The Alexander polynomial is a knot invariant, \index{knot!invariant} so it can
be used to determine when two knots are not the same.  For knots, it is
guaranteed to have integral powers.  For links, it might not:
\begin{lstlisting}[frame=single,framerule=0pt]
>> alexpoly(braid([1 1]))  % braid for the Hopf link

Error using braidlab.braid/alexpoly
Polynomial with fractional powers.  Try the 'uncentered' option or use the symbolic toolbox.
\end{lstlisting}
\index{Matlab!symbolic toolbox}%
Fractional powers cannot be represented with a \lstinline{laurpoly} object. %
\index{laurpoly@\lstinline{laurpoly}|)}%
\index{Laurent polynomials|)}%
In that case we can use the `uncentered' option, which yields the uncentered
...
which can represent fractional powers.
