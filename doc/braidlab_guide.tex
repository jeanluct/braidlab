...
\emph{crossing times}, %
\index{crossings!times|(}%
that is, the times at which two particles exchanged position aong the
projection line. %
\index{projection line}%
...
that retains the crossing times:
\begin{lstlisting}[frame=single,framerule=0pt]
>> b = databraid(XY,ti);
>> b.tcross(1:3)
...
\end{lstlisting}
\index{tcross@\lstinline{tcross}|)}%
There are exactly as many crossing times as generators in the braid.  Almost
all operations that can be done to \braid\ also work on a
\lstinline{databraid}, except \lstinline{compact}: %
\index{braid class@\braid\ class!compact@\lstinline{compact}}%
that method re-arranges the braid and so invalidates the crossing times.
\jlt{Rewrite a compact function for databraids that doesn't do that.}
\index{crossings!times|)}%
\index{databraid class@\lstinline{databraid} class|)}%
...
\begin{lstlisting}[frame=single,framerule=0pt]
>> taffy('4rods',pi/2)
Error using color_braiding (line 73)
