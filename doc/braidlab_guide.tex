...
\index{isotopy classes}
of homeomorphisms, so we can assign a topological entropy %
...
then plot this shortened braid as a braid diagram using \lstinline{plot(b)} to
produce Figure~\ref{fig:testdata_braid}.  The braid diagram allows us to see
...
\begin{equation}
  ...
  \label{eq:abvdef}
\end{equation}
which gives the \emph{loop coordinates}\index{loop!coordinates} (or
\emph{Dynnikov coordinates}) for the loop.  (Some authors such
