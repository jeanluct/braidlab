...
\jlt{This section is not indexed yet.}
...
\begin{center}
\subfigure[]{
  \includegraphics[height=.37\textheight]{efflinact1}
  \label{fig:efflinact1}
}\hspace{.5em}
\subfigure[]{
  \includegraphics[height=.37\textheight]{efflinact2}
  \label{fig:efflinact2}
}\hspace{.5em}
\subfigure[]{
  \includegraphics[height=.37\textheight]{efflinact3}
  \label{fig:efflinact3}
}
\end{center}
\caption{The plot produced by \lstinline{cycle(b,'plot')} for (a) \lstinline{b
    = braid([1 -2])}; (b) \lstinline{b = braid([1 2 3])}; (c) \lstinline{b =
    braid('psi',11)}.}
\end{figure}
%

The effective linear action has a very interesting behavior when a braid is
iterated on some initial curve.  
\begin{lstlisting}[frame=single,framerule=0pt]
>> [l,M] = b*l; l, full(M)
...
achieved a fixed point: running the same command again will change the loop,
but the linear action will remain the same forever.  \braidlab\ can automate
the iteration with the command \lstinline{cycle}.  Figure~\ref{fig:efflinact1}
shows the output of
\begin{lstlisting}[frame=single,framerule=0pt]
...
Perhaps the existence of this fixed point is not surprising, since the braid
$\sigma_1\sigma_2^{-1}$ corresponds to a pseudo-Anosov isotopy class.  In
general, instead of a fixed point we may find a \emph{limit cycle} of some
period.  Figure~\ref{fig:efflinact2} is produced by
\begin{lstlisting}[frame=single,framerule=0pt]
...
pseudo-Anosov braids can also achieve cycles: Figure~\ref{fig:efflinact2} is
the plot produced by
\begin{lstlisting}[frame=single,framerule=0pt]
>> b = braid('psi',11); [M,period] = cycle(b,'plot');
\end{lstlisting}
The period here is~$5$.  The matrix~$M$ is~$20$ by~$20$.  The braid
...
The largest eigenvalue of the matrix \lstinline{M} gives us the dilatation of
the braid, which in itself is not a real improvement over our earlier entropy
iterative algorithm.  However, with the matrix in hand we can find the
...
polynomials of the~$\psi$ braids, for testing purposes.)  Note that the factor
carrying the dilatation, \lstinline{x^3 - x^2 - 1}, appears in both
polynomials.  This is not always the case, though the dilatation has to be a
root of both polynomials.
