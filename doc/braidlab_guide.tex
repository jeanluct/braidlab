...
\item Currently the method \lstinline{subbraid} returns a \lstinline{braid}
  object, not a \lstinline{databraid}.  A future implementation will return a
  \lstinline{databraid} with the correct crossing times, so do not rely on
  this behavior.
\end{itemize} %
\index{crossing!times|)}%
