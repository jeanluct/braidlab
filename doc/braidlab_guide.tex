...
\lstinline{methods(braid)} to list all the methods in the class.)  There are
other ways to construct a \braid, such as using random %
...
\index{disk, punctured} Braids label \emph{isotopy classes} %
...
the braid.  For finite-order braids, this will converge to zero.  The
\emph{geometric complexity} %
...
of a braid~\citep{Dynnikov2007}, is defined in
terms of the $\log_2$ of the number of intersections of a set of curves with
the real axis, after one application of the braid:
\begin{lstlisting}[frame=single,framerule=0pt]
...
relative to the punctures.  In particular, the loops are \emph{essential}, %
...
\emph{multiloops}: loop coordinates can describe unions of disjoint
loops. \jlt{MRA suggests `You mention multiloops here in a sort of fragment
  thought at the end of the second paragraph.  Perhaps leave this concept out
  till section 2.4.'}
...
Now we can act on this loop with braids.  For example, we define the braid
\lstinline{b} to be~$\sigma_1^{-1}$ with~$5$ strings, corresponding to the~$5$
punctures, and then act on the loop \lstinline{l} by using the multiplication
operator:
%
\begin{figure}
\begin{center}
...
ans = (( 0  0  0  0 -1 -1 -1 -1 ))
\end{lstlisting}
This multiloop is depicted in Figure~\ref{fig:fundloops}.  Note that the
multiloop returned by \hbox{\lstinline{loop(5)}} actually has 6 punctures!
The rightmost puncture is meant to represent the boundary of a disk, or a base
point for the fundamental group on a sphere with $n$ punctures.  The loops
form a generating set for the fundamental group of the disk with $n$
...
Recall the command~\texttt{subbraid}
\index{braid class@\braid\ class!subbraid class@\lstinline{subbraid}}%
