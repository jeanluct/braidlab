...
\section{Limit cycles of loop coordinates}
\label{sec:cycles}
...
generator~$\sigma_\ip$ acting
on~$\abv=(\ac_1,\ldots,\ac_{\nn-2},\bc_1,\ldots,\bc_{\nn-2})$ as
\begin{subequations}
\begin{align}
...
The superscripts~${}^{+/-}$ are defined as
\begin{equation}
  \pos\fc \ldef \max(\fc,0),\qquad
  \neg\fc \ldef \min(\fc,0).
\end{equation}
(See~\citet{Thiffeault2010} for the update rules for the
generators~$\sigma_1$, $\sigma_{\nn-1}$, and the inverse generators.)  By
update rules we mean that the coordinates are updated to reflect the action of
the generator~$\sigma_\ip$ on a loop expressed in terms of coordinates.
Notice that the action~\eqref{eq:ur} is \emph{piecewise-linear} in the loop
coordinates.  Define the vector
\begin{equation}
  \Siv(\sigma_\ip;\abv) =
  %\siv^{(\ip)} =
  ...
  \pos{\bc_\ip} + \cc_{\ip-1} \,,\, \neg{\bc_{\ip-1}} - \cc_{\ip-1}\r),
\end{equation}
that is, the vector~$\Siv(\sigma_\ip;\abv)$ %$\siv^{(\ip)}$
contains five numbers corresponding to the sign of each quantity, which can
take the value~$-1$, $0$, or~$1$.  These quantities appear on the right-hand
side of~\eqref{eq:ur} with a~$+$ or~$-$ superscript.  If we know the
vector~$\Siv(\sigma_\ip;\abv)$, %~$\siv^{(\ip)}$,
we can uniquely reconstruct the linear action of~$\sigma_\ip$ on a given set
of coordinates.  For example, if~$\Siv(\sigma_\ip;\abv) = (1,1,1,1,1,1)$,
\jlt{Finish example.}
\begin{subequations}
\begin{align}
  \acnew_{\ip-1} &= \ac_{\ip-1} - \pos{\bc_{\ip-1}}
...
For a braid~$\gamma=\sigma_{\ip_1}\cdots\sigma_{\ip_k}$ acting on an initial
set of loop coordinates~$\abv$, we construct the vector
\begin{equation}
  \Siv(\sigma_{\ip_1}\cdots\sigma_{\ip_k};\abv) =
  \l(\Siv(\sigma_{\ip_1};\sigma_{\ip_1}\cdots\sigma_{\ip_k}\abv) \,,\,\ldots \,,\,
  \Siv(\sigma_{\ip_{k-1}};\sigma_{\ip_k}\cdot\abv) \,,\,
  \Siv(\sigma_{\ip_k};\abv) \r)
\end{equation}
with~$5k$ components.  The vector~$\Siv(\gamma;\abv)$, together with~$\gamma$
itself, allows us to reconstruct the net linear action of a braid.
